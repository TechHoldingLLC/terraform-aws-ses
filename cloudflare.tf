...
resource "cloudflare_dns_record" "ses_dkim_record" {
...
resource "cloudflare_dns_record" "ses_dmarc_record" {
...
resource "cloudflare_dns_record" "bounce_ses_domain_mail_from_mx" {
  count = length(var.cloudflare_zone_id) > 0 ? 1 : 0
  ...
  type     = "MX"
  ttl      = 600
  content  = "10 feedback-smtp.${local.region}.amazonses.com"
}

# Domain Mail From Cloudflare TXT record for SPF
resource "cloudflare_dns_record" "bounce_ses_domain_mail_from_txt" {
  count = length(var.cloudflare_zone_id) > 0 ? 1 : 0
